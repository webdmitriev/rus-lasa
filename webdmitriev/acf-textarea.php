      ...
      function addButtonsToTextarea($textarea) {
        ...
        if ($textarea.length && !$textarea.siblings('.acf-textarea-buttons').length) {
          ...
          var $btnSpan = $('<button type="button" class="button">SPAN</button>')
            ...
            var $btnList = $('<button type="button" class="button">UL/LI</button>')
              .attr('title', 'Список UL/LI')
              .click(function() {
                var selected = getSelection($textarea[0]);
                if (selected.text) {
                  // Разбиваем выделенный текст по строкам
                  var lines = selected.text.split(/\r?\n/).map(function(line) {
                    return '<li>' + line.trim() + '</li>';
                  }).join('');
                  wrapText($textarea, 'ul', selected, '', lines);
                } else {
                  insertText($textarea, '<ul>\n  <li>элемент списка</li>\n</ul>');
                }
              });

            $buttonGroup.append($btnA, $btnB, $btnSpan, $btnList);
        ...
        function wrapText($textarea, tag, selection, attributes, content) {
          var attrs = attributes ? ' ' + attributes : '';
          var inner = content !== undefined ? content : selection.text;
          var wrapped = '<' + tag + attrs + '>' + inner + '</' + tag + '>';
